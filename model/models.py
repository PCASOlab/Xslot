...
from video_SA.videosaur import configuration, losses, modules, optimizers, utils, visualizations
from video_SA.videosaur.data.transforms import Denormalize
from video_SA.videosaur.modules.presence_nn import Slot_Merger_Cosine_avg,Slot_Merger_Cosine
from model import model_operator
from math import sqrt
import model. model_operator_slots as slots_op
# from model.slots.temporal_slots_mask import Temporal_Binder
# from model.slots.temporal_slots import Temporal_Binder
 

from model.slots.temporal_slots_mask_last import Temporal_Binder as future_state_predit
from model.slots import temporal_slots_slide

from working_dir_root import Video_len, Evaluation_slots
...
def build(
    ...
    # if Using_SP:
    presence_nn = modules.build_presence_nn(model_config.Presence_prob_network,Sim_threshold=Sim_threshold)
    
    presence_nn = modules.MapOverTime2(presence_nn)
        # presence_nn = modules.MapOverTime2(presence_nn)
    ...
    if input_type == "image":
        ...
        # encoder = modules.IterOverTime(encoder)

        # decoder = modules.MapOverTime(decoder)
        if Using_SP:
            # decoder = modules.MapOverTime_mask(decoder)
            decoder = modules.IterOverTime_mask(decoder)

        else:
            decoder = modules.MapOverTime(decoder)


        # decoder = modules.IterOverTime(decoder)
...
class ObjectCentricModel(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        self.presence_nn = presence_nn
        self.Using_SP = Using_SP
        self.decoder = decoder
        self.target_encoder = target_encoder
        self.temporal_binder = Temporal_Binder(slot_dim=64, num_slots=7,num_frame=5)
        # self.future_state_prdt = future_state_predit(slot_dim=64, num_slots=7,num_frame=5)
        self.future_state_prdt = Temporal_Binder(slot_dim=64, num_slots=7,num_frame=5)

        self.single_slot_merger = Slot_Merger_Cosine_avg()
    ...
    def forward(self, inputs: Dict[str, Any], Using_bert =False,slot_ini = "rnn", Mask_feat = False,img_sim = False,epoch=0) -> Dict[str, Any]:
        ...
        B, D, HW, F= features.size()
        H = W =int ( sqrt (HW))
        if Mask_feat == True :
        # Pure_down_pool = nn.AvgPool3d((1,2,2),stride=(1,2,2))
            this_feature = features.reshape(B,D,H,W,F) 
            this_feature = this_feature.permute(0, 4, 1, 2, 3)  # Now shape is [1, 64, 100, 14, 14]s
            this_feature = model_operator.hide_patch (this_feature, patch_num=HW)
            # this_feature = Pure_down_pool(this_feature)
            this_feature = this_feature.reshape(B,F,D,HW)
            this_feature =  this_feature.permute(0, 2, 3,1)  # Now shape is [1, 100, 14* 14,64]s
        else:
            this_feature = features  #.detach().clone()
        ...
        if slot_ini== "rnn":
            processor_output = self.processor(slots_initial, this_feature, rnn= True )
        elif slot_ini== "videosaur":
            processor_output = self.processor(slots_initial, this_feature, rnn= True,Next_state_predict="videosaur" )
        elif slot_ini== "binder":
            processor_output = self.processor(slots_initial, this_feature, rnn= True,Next_state_predict="binder",model=self.temporal_binder )
        elif slot_ini== "binder+merger":
            processor_output = self.processor(slots_initial, this_feature, rnn= True,Next_state_predict="binder+merger",model=self.temporal_binder,merger=self.single_slot_merger )
        elif slot_ini== "binder2+merger":
            processor_output = self.processor(slots_initial, this_feature, rnn= True,Next_state_predict="binder+merger",model=self.future_state_prdt,merger=self.single_slot_merger )
        else:
            processor_output= self.processor(slots_initial, this_feature,rnn=False)

        # processor_output = self.processor(slots_initial, this_feature)

        # with torch.no_grad():

        slots = processor_output["state"] 
        # slots = slots_op.align_permutations_across_n_frames(slots,n=1)
        ...
        if (self.presence_nn is not None) and self.Using_SP == True:
            ...
            if not isinstance(presence_outs, tuple):
                 
                Presence_prob = presence_outs

            else:
                (slots,Presence_prob) = presence_outs

        else:
            Presence_prob = None

        # slots = slots_op.align_permutations_across_n_frames(slots,n=3)
        # slots = slots_op.align_permutations_across_n_frames(slots,n=2)
        # slots = slots_op.add_noise_to_latents(slots,noise_std=0.05)
        
        processor_output["state"] = slots
        # slots = slots_op.align_permutations_across_frames(slots)

        # affinity_matrix = slots_op.compute_cosine_similarity_matrix(slots)

        # # Step 2: Find optimal permutations for each batch using Hungarian algorithm
        # permutations = slots_op.find_optimal_permutation(affinity_matrix)
        # # permutations = slots_op.compute_soft_permutation(affinity_matrix)



        # # Step 3: Apply the optimal permutation to reorder the latent vectors
        # permuted_latent_vectors = slots_op.apply_permutation(slots, permutations)
        # slots = permuted_latent_vectors
        # with torch.no_grad():
        if Presence_prob is None:
            decoder_output = self.decoder(slots)
        else:
            # if Evaluation_slots:
            #     decoder_output = self.decoder(slots.detach().clone(),Presence_prob.detach().clone())
            # else:
            decoder_output = self.decoder(slots,Presence_prob)

        outputs ["decoder"] = decoder_output
        outputs ["processor"] = processor_output
        outputs ["presence_p"] = Presence_prob
